...
static void mctp_usb_in_complete(struct urb *urb)
{
	...
	while (skb) {
		...
		struct mctp_usb_hdr *hdr;
		uint8_t pkt_len; /* length of MCTP packet, no USB header */
		...
		if (hdr->len > skb->len + sizeof(struct mctp_usb_hdr)) {
			dev_dbg(&mctp_usb->usbdev->dev,
