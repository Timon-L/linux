...
static int mctp_rx_submit(struct f_mctp *mctp, struct usb_ep *ep)
{
	...
	req = alloc_ep_req(ep, MCTP_USB_XFER_SIZE);
	if (!req) {
		return -1;
	}
	...
	req->complete = mctp_usbg_out_ep_complete;
	req->buf = skb->data;
	req->context = skb;
...
static int mctp_usbg_enable(struct usb_composite_dev *cdev, struct f_mctp *mctp)
{
	...
	rc = mctp_rx_submit(mctp, mctp->out_ep);
	if (rc) {
